...
class Server {
  ...
  async middlewares(app) {
    app.use(cors()) // Utilização da função cors no servidor
    app.use(express.json()) // Habilitar entrada de dados com o formato json no servidor
  }

  async database() {
    try {
      await connection.authenticate(); // Tentativa de conexão com o data base
  ...
  async initializeServer(app) {
    ...
    app.listen(PORT_API, () => console.log(`Servidor executando na porta ${PORT_API}`)) // Execução do servidor na porta definida
  }
}

module.exports = { Server } // Exportação da Classe do servidor